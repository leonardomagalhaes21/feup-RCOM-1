...
void set_rt_priority(void) {
    // macOS doesn't support SCHED_FIFO, but we can still set process priority
    int priority = -10; // Higher priority (lower number) on macOS
...
int main(int argc, char *argv[])
{
    ...
    system("socat -dd PTY,link=" TXDEV ",mode=777,raw,echo=0 PTY,link=/tmp/emulatorTx,mode=777,raw,echo=0 &");
    sleep(1);
    printf("\n");

    system("socat -dd PTY,link=" RXDEV ",mode=777,raw,echo=0 PTY,link=/tmp/emulatorRx,mode=777,raw,echo=0 &");
    sleep(1);
    ...
    int fdTx = openSerialPort("/tmp/emulatorTx", &oldtioTx, &newtioTx);
    ...
    int fdRx = openSerialPort("/tmp/emulatorRx", &oldtioRx, &newtioRx);
